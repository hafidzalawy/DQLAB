...
SELECT StudentID, 
	   SUBSTRING_INDEX(email, '@', 1) as Name -- Query pada MySQL
FROM students;

SELECT StudentID, 
	   SPLIT_PART(email, '@', 1) as Name -- Query pada PostgreSQL
FROM students;
...
---Query PostgreSQL
SELECT EXTRACT(MONTH FROM order_date) AS order_month, SUM(item_price) AS total_price,
    CASE
        WHEN SUM(item_price) >= 30000000000 THEN 'Target Achieved'
        WHEN SUM(item_price) <= 25000000000 THEN 'Less performed'
        ELSE 'Follow Up'
